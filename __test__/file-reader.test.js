...
describe('file-reader.js', () => {
        test('To see if the the input is a string or array',()=>{
            return fileReader.readFirstNCharactersAsync([pathOne, pathTwo, pathThree], (error, data) =>{
                expect(value).notEqual(typeof 'string');
                expect(value).toEqual(typeof 'array');
                console.log(value);
            })
    });

        test('Call text files in order reguardless of size', () => {
            return fileReader.readFirstNCharactersAsync([pathOne, pathTwo, pathThree], (error, data) => {
